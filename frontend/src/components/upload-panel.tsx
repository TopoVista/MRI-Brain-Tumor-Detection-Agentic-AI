      ...
      <CardContent className="grid gap-5 p-6">
        <form action={handleSubmit} className="space-y-4">
          <label className="group relative flex cursor-pointer flex-col items-center justify-center overflow-hidden rounded-lg border border-dashed border-border bg-secondary px-6 py-10 text-center transition hover:border-sky-400/22 hover:bg-[rgba(17,28,41,0.98)]">
            ...
            <span className="mt-4 text-lg font-semibold text-white">Drop MRI image or browse</span>
          ...
          {previewUrl ? (
            <div className="overflow-hidden rounded-lg border border-border bg-card">
              <div className="flex items-center justify-between border-b border-border px-4 py-3">
                ...
                {isUploading ? <span className="text-xs font-semibold uppercase tracking-[0.22em] text-sky-300">Analyzing</span> : null}
              </div>
          ...
          <Button type="submit" className="w-full" disabled={isUploading}>
            <span className="button-content relative z-10 flex items-center gap-2">
              ...
              {isUploading ? "Running workflow..." : "Start workflow"}
            </span>
        ...
        {isUploading ? (
          <div className="rounded-md border border-sky-400/22 bg-[rgba(17,39,51,0.66)] px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-medium text-sky-100">Workflow in progress. Generating preview states and report.</p>
            </div>
          </div>
        ) : null}
        ...
        <div className="rounded-md border border-border bg-secondary px-4 py-4">
          <div className="flex items-center gap-2 text-slate-200">
            ...
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-sky-200/68">Clinical positioning</p>
          </div>
          <p className="mt-2 text-sm leading-6 text-slate-300">Decision support only. Final interpretation should remain with qualified clinical review.</p>
        </div>
